...
void Grouping::Classification_FindContours(Mat *segmentacao)
{
    ...
    pontos.clear();
    findContours(*segmentacao,contours,hierarchy,RETR_LIST,CHAIN_APPROX_NONE);
    int soma_x=0;
    int soma_y=0;
...
void Grouping::Classification_kmeans(Mat *segmentacao)
{
          ...
          if(nonZeroCoordinates.rows > number){
              ...
              kmeans(nonZeroCoordinates, incremento_centro, labels, TermCriteria(TermCriteria::COUNT|TermCriteria::EPS,5, 0.001), attempts, KMEANS_RANDOM_CENTERS, centers);
