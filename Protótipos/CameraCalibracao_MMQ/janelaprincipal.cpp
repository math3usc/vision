...
{
    ui->setupUi(this);
    ui->frame->setScaledContents(false);
...
void JanelaPrincipal::Mouse_pressed()
{
    ...
    if(modo_operate == CORTAR)
    {
        ...
        if(cont < 2)
        {
            ...
            roi = Rect2d(pontos_corte[0],pontos_corte[1]);
...
void JanelaPrincipal::WorkMetodo()
{
    ...
    if(cont == 2) applyCorte = true;
    if(applyCorte) img = img(roi);
    ...
    if(guia_enable == 0)
    {
        ...
        image = QImage((uchar*) img.data, img.cols, img.rows,img.step, QImage::Format_RGB888);
     ...
     }else if(guia_enable == 2)
    {
        ...
        image = QImage((uchar*) guia.data, guia.cols, guia.rows,guia.step, QImage::Format_RGB888);
        ui->frame->setPixmap(QPixmap::fromImage(image));
    }
